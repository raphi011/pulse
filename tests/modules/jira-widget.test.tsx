...
describe("JqlWidget", () => {
  it("renders each issue as a link to its browse URL", () => {
    render(<JqlWidget data={data} config={config} saveConfig={noop} />);
  ...
  it("shows the status name and assignee initials, with — for unassigned", () => {
    ...
    render(<JqlWidget data={two} config={config} saveConfig={noop} />);
  ...
  it("shows an empty message when there are no issues", () => {
    render(<JqlWidget data={{ issues: [] }} config={config} saveConfig={noop} />);
