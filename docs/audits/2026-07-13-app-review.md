...
- [x] **F3 (MEDIUM)** — Next-meeting can miss the real next meeting: `src/modules/gws/calendar.ts:84-99` — `maxResults: 20` applies to raw events pre-filter (all-day/declined/solo count against it), no `nextPageToken` follow-up → busy day shows "no more meetings today".
...
- [x] **F8 (MEDIUM)** — Bookmarks Enter-key double-add: `src/modules/bookmarks/widgets/bookmarks-widget.tsx:176` — Enter calls `add()` which lacks an `if (saving) return` guard (only the Save button is disabled) → duplicate rows.
