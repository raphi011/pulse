...
**Status (2026-07-10):** items 1–6 resolved. Only the deferred merge-action work remains.

## Important

1. ✅ **DONE** — **"Not configured" empty state** *(spec gap — spec promised "teaching empty states")*
...
2. ✅ **DONE** — **Silent per-repo partial failure** *(spec gap — spec said "surface a non-fatal note if some targets errored")*
...
3. ✅ **DONE** — **`repo` config is unvalidated `z.string()`** (`manifest.ts` — failingActions/dependabot `repos`).
...
4. ✅ **DONE** — **Config drift until reload in the save dialog** (`configure-dialog.tsx`).
...
5. ✅ **DONE** — **`maxBuffer` overflow misclassified as `timeout`** (`cli.ts`).
...
6. ✅ **DONE** (with item 1) — **Body `config` generic left as `unknown`** in the three widget bodies despite `MyPrsConfig`/etc.
